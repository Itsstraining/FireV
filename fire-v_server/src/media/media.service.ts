...
export class MediaService {
  ...
  cutVideo(file: Express.Multer.File) {
    ...
    return new Promise<Express.Multer.File>((resolve, rejects) => {
      mkdirSync(`./uploads/videos/cvt/${file.filename}-conv`);
      renameSync(`./uploads/videos/${file.filename}`, `./uploads/videos/cvt/${file.filename}-conv/${file.filename}`);
      ffmpeg()
        .addInput(`./uploads/videos/cvt/${file.filename}-conv/${file.filename}`)
        .output(`./uploads/videos/cvt/${file.filename}-conv/main.m3u8`)
        .outputOptions([
          // '-g 60',
          '-f hls',
          ...
          '-hls_segment_filename', `./uploads/videos/cvt/${file.filename}-conv/fileSequence%d.section`
        ])
        ...
        .on('end', async function () {
          ...
          unlinkSync(`./uploads/videos/cvt/${file.filename}-conv/${file.filename}`);
          resolve(file);
        })
        .run();
    });
